...
namespace Projet
{
    public partial class SupprimerJeuDlg : Form
    {
        private Lesjeux lj;
        private List<string> nom;
        public SupprimerJeuDlg(Lesjeux l)
        {
            InitializeComponent();
            this.lj = l;
            lj.Trie();
        ...
        private void Maj_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            int row = Maj.CurrentCell.RowIndex;
            if (row >= 0)
            {
                Jeu j = lj.Getindice(row);
                nom.Add(j.Nom);
            }
           
        }

        private void suppimer_Click(object sender, EventArgs e)
        {
